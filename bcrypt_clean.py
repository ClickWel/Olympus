#!/usr/bin/env python3
"""
Clean bcrypt $2$ implementation.
Based on OpenBSD bcrypt and Wikipedia.
"""
import struct

# Blowfish S-boxes (initial values)
Sbox = [
...
BCRYPT_CTEXT = b'OrpheanBeholderScryDoubt'  # 24 bytes
B64 = './ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'

def bf_F(S, x):
    return ((S[0][x >> 24] + S[1][(x >> 16) & 0xFF]) & 0xFFFFFFFF) ^ S[2][(x >> 8) & 0xFF] + S[3][x & 0xFF] & 0xFFFFFFFF

def bf_encrypt(P, S, xl, xr):
    for i in range(16):
        xl ^= P[i]
        xr ^= bf_F(S, xl)
        xr ^= P[i+1]
...
def bcrypt_hash(password, salt, cost):
    # Initialize
    P = list(Pbox)
    S = [Sbox[i*256:(i+1)*256] for i in range(4)]

    # Key setup (password is null-terminated)
    if isinstance(password, str):
        password = password.encode('utf-8')
    key = password + b'\x00'
    key_len = len(key)

    # Expand P with password
    for i in range(18):
        data = 0
        for j in range(4):
    ...
    for i in range(0, 18, 2):
        ...
        xl, xr = bf_encrypt(P, S, xl, xr)
        P[i] = xl
        P[i+1] = xr

    # Expand S-boxes with salt
    for i in range(4):
    ...
    for _ in range(1 << cost):
        for j in range(len(blocks)):
            blocks[j][0], blocks[j][1] = bf_encrypt(P, S, blocks[j][0], blocks[j][1])
...
def gen_hash_line(password, salt_bytes, cost):
    raw_hash = bcrypt_hash(password, salt_bytes, cost)
    salt_b64 = b64_encode(salt_bytes)
    hash_b64 = b64_encode(raw_hash)
    return f'root:$2${cost:02d}${salt_b64}{hash_b64[:31]}'

if __name__ == '__main__':
    # Test with zero salt, cost 4
    print("Test: empty password, zero salt, cost 4")
    result = gen_hash_line(b'', b'\x00' * 16, 4)
    print(f'  {result}')

    # Test with JgiaOAai salt, cost 4
    ...
    print(f"\nJgiaOAai decoded: {jgia.hex()}")
    salt = jgia + b'\x00' * (16 - len(jgia))
    print(f"Salt bytes: {salt.hex()}")
    result2 = gen_hash_line(b'', salt, 4)
    print(f'  {result2}')
