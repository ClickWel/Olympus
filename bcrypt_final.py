#!/usr/bin/env python3
"""
Bcrypt implementation with test vector verification.
"""
import struct

# Blowfish S-boxes (initial - from OpenBSD bcrypt)
Sbox = [
...
def bcrypt(pwd, salt, cost):
    # Init
    P = list(Pbox)
    S = [Sbox[i*256:(i+1)*256] for i in range(4)]

    # Key = password null-terminated
    key = pwd + b'\x00'
    key_len = len(key)
    ...
    out = b''
    for xl, xr in blocks:
        out += xl.to_bytes(4, 'big') + xr.to_bytes(4, 'big')
    return out
...
def gen_line(pwd, salt_bytes, cost):
    raw = bcrypt(pwd, salt_bytes, cost)
    salt_b64 = b64_encode(salt_bytes)
    hash_b64 = b64_encode(raw)
    return f'root:$2${cost:02d}${salt_b64}{hash_b64[:31]}'

if __name__ == '__main__':
    # Test: empty password, JgiaOAai salt, cost 4
    ...
    # Generate hash
    result = gen_line(b'', salt, 4)
    print(f'Result: {result}')

    # Also try cost 5, 6
    for cost in [5, 6]:
        r = gen_line(b'', salt, cost)
        print(f'Cost {cost}: {r}')
