#!/usr/bin/env python3
"""
Pure Python bcrypt $2$ implementation.
...
def F(P, S, x):
    ...
    c = S[2][(x >> 8) & 0xFF]
    d = S[3][x & 0xFF]
    step1 = (a + b) & 0xFFFFFFFF
    step2 = step1 ^ c
    step3 = (step2 + d) & 0xFFFFFFFF
    return step3
...
def bcrypt(password, salt, cost):
    # Initialize
    P = list(P_INIT)
    S = [list(S_INIT[i*256:(i+1)*256]) for i in range(4)]
    ...
    # Expand key with salt (second pass)
...
def gen_hash(password, salt_bytes, cost):
    raw = bcrypt(password, salt_bytes, cost)
...
if __name__ == '__main__':
    # Test with zero salt
    salt = b'\x00' * 16
    result = gen_hash(b'', salt, 4)
    print(f'Zero salt, cost 4: {result}')

    # Test with JgiaOAai salt
    jgia_bytes = b64_decode('JgiaOAai')
    salt2 = jgia_bytes + b'\x00' * (16 - len(jgia_bytes))
    result2 = gen_hash(b'', salt2, 4)
    print(f'JgiaOAai salt, cost 4: {result2}')
