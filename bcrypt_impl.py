#!/usr/bin/env python3
"""
Pure Python bcrypt implementation.
Based on OpenBSD bcrypt and Wikipediabcrypt articles.
"""
import struct

# Blowfish S-boxes (initial values - from OpenBSD)
S_ORIG = [
    ...
    0xB3B8A6C1, 0xCCBF4C31, 0x7C9F1F2A, 0x6A5C63F6, 0xBC20400E, 0xFA8D5C38,
    0x6C8A9992, 0x2EAp3CB7, 0x3BCCB21A, 0x19EDE73A, 0x29C1755E, 0x3EA3B71E,
    0x2E4B4B7A, 0x30E0DAA9, 0x3602FA41, 0x2AABE2C1, 0x532E16D0, 0x492339C5,
    0x0A0AC82E, 0x49E3E57A, 0x1F0AC9AE, 0x3BDF58D6, 0x03A396FE, 0x80C62557,
    ...
    0xEA758B58, 0x5C119B52, 0xEDD7D3C8, 0x6CB20C8E, 0x0C8D8131, 0x59301C63,
    ...
    0x6FAC6051, 0x0C9851EC, 0x2E2C8486, 0x32F3B827, 0xACA51B18, 0x5A84C371,
    0x5A84C371, 0x5A84C371, 0x5A84C371, 0x5A84C371, 0x5A84C371, 0x5A84C371,
]

# Fix the corrupted values
S_ORIG = [0xd1310ba6, 0x98dfb5ac, 0x2ffd72db, 0xd01adfb7, 0xb8e1afed, 0x6a267e96,
    0xba7c9045, 0xf12c7f99, 0x24a19947, 0xb3916cf7, 0x0801f2e2, 0x858efc16,
    0x636920d8, 0x71574e69, 0xa458fea3, 0xf4933d7e, 0x0d95748f, 0x728eb658,
    0x718bcd58, 0x82154aee, 0x7b54a41d, 0xc25a59b5, 0x9c30d539, 0x2af26013,
    0xc5d1b023, 0x286085f0, 0xca417918, 0xb8db38ef, 0x8e79dcb0, 0x603a180e,
...
def bcrypt(password, salt, cost):
    """Generate bcrypt hash of password with given salt and cost."""
    # Initialize state
    P = list(P_ORIG)
    S = [list(S_ORIG[i*256:(i+1)*256]) for i in range(4)]
    ...
    key_len = len(key)
    for i in range(18):
        # Build 32-bit value from key bytes (cyclically)
        data = 0
        for j in range(4):
            data = (data << 8) | key[(i * 4 + j) % key_len]
        P[i] ^= data

    # Use salt to expand key
    salt_len = len(salt)
    def get_salt_byte(idx):
        return salt[idx % salt_len]

    xl, xr = 0, 0
    for i in range(0, 18, 2):
        xl = 0
        for j in range(4):
            xl = (xl << 8) | get_salt_byte(i * 4 + j)
        xr = 0
        for j in range(4):
            xr = (xr << 8) | get_salt_byte(i * 4 + j + 4)
        xl ^= xl  # XOR with self (no-op, originally XOR with salt)
        xr ^= xr
    ...
    for i in range(4):
        for j in range(0, 256, 2):
            xl = 0
            for k in range(4):
                xl = (xl << 8) | get_salt_byte((i * 256 + j) * 4 + k)
            xr = 0
            for k in range(4):
                xr = (xr << 8) | get_salt_byte((i * 256 + j + 1) * 4 + k)
            xl ^= xl  # XOR with self (no-op)
            xr ^= xr
            xl, xr = blowfish_encipher(P, S, xl, xr)
            S[i][j] = xl
            S[i][j+1] = xr
...
def bcrypt_hash_line(password, salt_bytes, cost):
    """Generate full bcrypt hash line."""
    raw_hash = bcrypt(password, salt_bytes, cost)
