...
def parse_pcap(path):
    ...
    for key, segments in tcp_streams.items():
        ...
        print(f"Processing part {part_num}, boundary {boundary[:20]}...")

        # Split by boundary
        parts = text.split(f'--{boundary}')
        for part in parts:
            if 'Content-Transfer-Encoding: base64' in part:
                # Extract base64 content: after the blank line following base64 header
                lines = part.split('\n')
                b64_lines = []
                capture = False
                for line in lines:
                ...
                print(f"Part {part_num}: base64 length {len(b64_data)}")
